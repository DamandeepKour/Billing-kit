...
export class InvoicePdfGenerator {
  ...
  generateInvoicePdf(input: GeneratePdfInput): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      doc
        .fontSize(12)
        .text(`Grand Total: ${formatAmount(invoice.total, invoice.currency)}`, {
          align: "right",
        });
      if (invoice.notes) {
        doc.moveDown();
